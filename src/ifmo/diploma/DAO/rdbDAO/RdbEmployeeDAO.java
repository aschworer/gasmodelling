...
import ifmo.diploma.DAO.EmployeeDAO;
import ifmo.diploma.DAO.GettingDataFailedException;
import ifmo.diploma.model.*;
        ...
        try {
            ...
            insertEmployee = conn.prepareStatement(
                    "INSERT INTO employees (firstname, lastname, middlename, passport_no, passport_ser," +
                            ...
                            " charachteristics, current_position, current_department, resume_filename, photo_filename, systemLogin, systemPass, usergroup)\n" +
                            "  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
            updateEmployee = conn.prepareStatement(
                    "update employees set firstname=?, lastname=?, middlename=?, passport_no=?, passport_ser=?," +
                            " passport_received_when=?, passport_received_from=?," +
                            "  passport_received_where=?, passport_copy_filename=?," +
                            " charachteristics=?, current_position=?, current_department=?, resume_filename=?, photo_filename=?, systemLogin=?, systemPass=?, usergroup=? where id =?;");
    ...
    public Integer saveEmployee(Employee employee) throws GettingDataFailedException {
        ...
        try {
            if (employee.getId() == 0) {
                ...
                insertEmployee.setString(15, employee.getSystemLogin());
                insertEmployee.setString(16, employee.getSystemPass());
                insertEmployee.setString(17, employee.getSystemPass());
                ...
                updateEmployee.setString(15, employee.getSystemLogin());
                updateEmployee.setString(16, employee.getSystemPass());
                updateEmployee.setInt(17, id);
    ...
    public Employee getEmployee(int employeeID) throws GettingDataFailedException {
        try {
            ...
            while (rs.next()) {
                ...
                newEmployee.setSystemLogin(rs.getString("systemLogin"));
                newEmployee.setSystemPass(rs.getString("systemPassMD5"));
                newEmployee.setGroupId(new Long(rs.getInt("usergroup")));
