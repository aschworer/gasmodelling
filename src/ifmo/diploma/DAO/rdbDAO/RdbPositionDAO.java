...
import ifmo.diploma.DAO.PositionDAO;
...
public class RdbPositionDAO implements PositionDAO {
    ...
    public RdbPositionDAO(Connection conn) throws GettingDataFailedException {
        try {
            ...
            selectGroups = conn.prepareStatement("select * from groups order by name;");
    ...
    public List<UserGroup> getAllGroups() throws GettingDataFailedException {
        ArrayList<UserGroup> result = new ArrayList<UserGroup>();
        try {
            ResultSet rs = selectGroups.executeQuery();
            while (rs.next()) {
                result.add(UserGroup.getByName(rs.getString("name")));
            }
            rs.close();
        } catch (SQLException e) {
            System.err.println("\n---SQLException caught---\n" + e.getMessage());
